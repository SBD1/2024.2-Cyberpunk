...
DECLARE
    nova_mochila_id INT;
BEGIN
    -- Inserir uma nova mochila com capacidade 20 para o CyberLutador
    INSERT INTO Mochila (capacidade, fk_cyberlutador)
    VALUES (20, NEW.idCyberLutador) 
    RETURNING idMochila INTO nova_mochila_id; -- Retorna o id da nova mochila inserida
    
    -- Atualizar o CyberLutador, associando à nova mochila (opcional, se necessário)
    UPDATE CyberLutador
    SET fk_mochila = nova_mochila_id
    WHERE idCyberLutador = NEW.idCyberLutador;

    RETURN NEW;
END;
$criar_mochila$ LANGUAGE plpgsql;

-- Criar trigger para a função criar_mochila
